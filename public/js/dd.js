...
import { Object3D } from "./modules/Object3D.js";
import { Group } from "./modules/Group.js";
import { Matrix } from "./modules/Matrix.js";
import {
  rotateAxes,
  rotatePoints,
  multiplyMatrixVectors,
  multiplyMatrixVectors2,
  MMulti,
  RotateX,
  RotateY,
  RotateZ,
...
function getAppliedRotationDelta(previousAngle, nextAngle) {
  let delta = Math.floor(nextAngle) - Math.floor(previousAngle);
...
function applyRotationDelta(matrix, rx = 0, ry = 0, rz = 0) {
  let resultMatrix = matrix;

  if (rx !== 0) resultMatrix = M.m_rotate(0, resultMatrix, rx);
  if (ry !== 0) resultMatrix = M.m_rotate(1, resultMatrix, ry);
  if (rz !== 0) resultMatrix = M.m_rotate(2, resultMatrix, rz);
...
function eventKeyDown(ev) {
  ...
  let num = Number.parseInt(keyPressed, 10) - 1;
  if (!Number.isNaN(num) && num >= 0 && num < Cubes.objects.length) {
    selected = num;
  }
  ...
  switch (ev.code) {
    ...
    case "KeyX":
      ...
      Cubes.objects[selected].rotMatrix = newRot(0, [
        ...Cubes.objects[selected].rotMatrix,
      ]);
      Cubes.objects[selected].rotations[0] = wrapRotation(
        Cubes.objects[selected].rotations[0] + 5,
      );
    ...
    case "KeyY":
      ...
      Cubes.objects[selected].rotMatrix = newRot(1, [
        ...Cubes.objects[selected].rotMatrix,
      ]);
      Cubes.objects[selected].rotations[1] = wrapRotation(
        Cubes.objects[selected].rotations[1] + 5,
      );
      update();
      break;
    case "KeyZ":
      //objects[selected].setRotationMatrix(2);
      Cubes.objects[selected].rotMatrix = newRot(2, [
        ...Cubes.objects[selected].rotMatrix,
      ]);
      Cubes.objects[selected].rotations[2] = wrapRotation(
        Cubes.objects[selected].rotations[2] + 5,
      );
      update();
      break;
    case "ArrowLeft":
...
function update() {
  ...
  let pos = [];
  let cPos = [
    Cubes.transMatrix[0][3],
    Cubes.transMatrix[1][3],
    Cubes.transMatrix[2][3],
  ];
  ...
  for (let i = 0; i < Cubes.objects.length; i++) {
    ...
    //obj.rotatedPoints =
    obj.rotatedPoints = M.m_mul_array_v3(obj.rotatedPoints, [
      ...obj.scaleMatrix,
    ]);
    ...
    obj.rotatedPoints = multiplyMatrixVectors(
      [...obj.rotatedPoints],
      worldMatrix,
    );
    obj.rotatedPosition = multiplyMatrixVectors(
      [...obj.rotatedPosition],
      worldMatrix,
    );
    obj.rotatedPoints = multiplyMatrixVectors(
  ...
  canvasGrid.drawBackgroundImage(
    imgGround,
    0,
    0,
    canvasGrid.width,
  ...
  for (let i = 0; i < Cubes.objects.length; i++) {
    ...
    if (z > 1.5) {
      canvasTop.drawTriangleSides(
        Cubes.objects[i].triangles,
        Cubes.objects[i].rotatedPoints,
        vCamera,
      );
    }
  }
...
function setup() {
  ...
  for (let i = 0; i < maxObj; i++) {
    ...
    obj = new Object3D(name, [0, 0, 0, 1], [num1, num2, num3, 1]);
    obj.ground = num2;
    obj.rotations = [0, 0, 0];
...
function animate(timestamp) {
  ...
  anim_data.delta = timestamp - anim_data.then;
  if (anim_data.delta >= anim_data.interval) {
    anim_data.then = timestamp - (anim_data.delta % anim_data.interval);
    //real anim
    let obj = {};
    ...
    const previousGroupRotationY = Cubes.rotations[1];
    Cubes.rotations[1] = wrapRotation(Cubes.rotations[1] + 0.5);
    const deltaGroupRotationY = getAppliedRotationDelta(
      previousGroupRotationY,
      Cubes.rotations[1],
    );
    Cubes.groupMatrix = applyRotationDelta(
      Cubes.groupMatrix,
      0,
      deltaGroupRotationY,
      0,
    ...
    for (let i = 0; i < Cubes.objects.length; i++) {
      ...
      str = obj.anim.r * Math.cos(degToRad(obj.anim.rdeg));
      //str = obj.anim.r;
      const previousRotX = obj.rotations[0];
      const previousRotY = obj.rotations[1];
      const deltaRotX = obj.anim.step / 3;
      const deltaRotY = obj.anim.step;
      obj.rotations[0] = wrapRotation(obj.rotations[0] + deltaRotX);
      obj.rotations[1] = wrapRotation(obj.rotations[1] + deltaRotY);
      //console.log("r: "+str);
      obj.rotations[2] = wrapRotation(obj.rotations[2]);
      const appliedRotX = getAppliedRotationDelta(previousRotX, obj.rotations[0]);
      const appliedRotY = getAppliedRotationDelta(previousRotY, obj.rotations[1]);
      ...
      obj.transMatrix = M.m_make_translate(
        obj.position[0],
        obj.position[1],
        obj.position[2],
      );
      ...
      obj.rotMatrix = applyRotationDelta(obj.rotMatrix, appliedRotX, appliedRotY, 0);
    }
    update();
  }
}
...
function rp(mini = 1, maxi = 2) {
  ...
  return (
    (Math.exp(Math.random() * (Math.log(maxi) - Math.log(mini))) * mini) / 2.0
  );
}
